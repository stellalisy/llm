...
class AsyncBaseAgent(BaseAgent):
    ...
    def interact(self, prompt, temperature=None, max_tokens=None):
        output = self._generate(prompt, temperature=temperature, max_tokens=max_tokens)
        response = self._postprocess_output(output)
        return response

    async def _batch_generate(self, prompts, temperature=None, max_tokens=None):
        loop = asyncio.get_running_loop()
        completions = await asyncio.gather(*[
            loop.run_in_executor(self.executor, self._generate, prompt, temperature, max_tokens)
    ...
    def batch_interact(self, prompts, temperature=None, max_tokens=None):
        while True:
            try:
                outputs = asyncio.run(self._batch_generate(prompts, temperature=None, max_tokens=None))
                responses = [self._postprocess_output(output) for output in outputs]
